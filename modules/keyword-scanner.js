...
window.startKeywordScan = async function() {
    ...
    resultsDiv.insertAdjacentHTML('afterbegin', `<div id="${loadId}" class='flex flex-col items-center justify-center my-4 gap-2 animate-pulse'><div class='w-6 h-6 border-2 border-[#00ff41] border-t-transparent rounded-full animate-spin'></div><div class='text-[10px] uppercase'>Generiere SEO Dashboard...</div></div>`);
        ...
        const resultHTML = `
            <div class="space-y-4 font-mono mb-8">
                ...
                <div class="bg-black/40 border border-green-900/30 p-3 text-[11px] text-white">
                    ...
                    <ul class="space-y-1">${d.longtails.map(kw => `<li><span class="text-green-600 mr-2">></span>${kw}</li>`).join('')}</ul>
                </div>
    ...
    } catch (e) { document.getElementById(loadId).innerHTML = `<div class="text-red-500 text-xs p-4 border border-red-900">API Timeout.</div>`; }
}
