...
window.startTrendScan = async function() {
    ...
    resultsDiv.insertAdjacentHTML('afterbegin', `<div id="${loadId}" class='flex flex-col items-center justify-center my-4 gap-2 animate-pulse'><div class='w-8 h-8 border-2 border-[#00ff41] border-t-transparent rounded-full animate-spin'></div><div class='text-[10px] uppercase'>Analysiere Profitabilität...</div></div>`);
    ...
    try {
        ...
        // NEU: Feedback Prozentwert auslesen
        ...
        const resultHTML = `
            <div class="bg-black/60 border border-green-900/50 p-4 font-mono mb-6">
                ...
                <div class="grid grid-cols-2 gap-4 text-[11px] mb-4 border-b border-green-900/30 pb-4">
                    ...
                    <div><div class="text-green-500/60 uppercase mb-0.5">FESTPREIS-RATE</div><div class="text-white">${buyItNowP}% BuyItNow</div></div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="mt-2 text-[10px] grid grid-cols-2 gap-2 bg-black p-2 border border-green-900/50">
    ...
    } catch (e) { document.getElementById(loadId).innerHTML = `<div class="text-red-500 p-4 text-xs">SYSTEM_ERROR</div>`; }
}
