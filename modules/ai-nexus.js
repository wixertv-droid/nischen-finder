...
async function startAiScan() {
    ...
    if (!token) {
        resultsDiv.innerHTML = "<div class='text-red-500 text-[11px] border border-red-900 p-3 bg-red-900/20 text-center mt-4 font-mono'>[FATAL_ERR] NO_TOKEN_FOUND.<br>Bitte im Reiter SYSTEM eintragen.</div>";
        return;
    }
    if (!input) return;

    // UI-Feedback: Lade-Animation im Jarvis-Stil
    ...
    try {
        const promptText = `Du bist ein strategischer E-Commerce Analyst im Cyberpunk-Stil. 
        Analysiere die Nische: ${input}. 
        Antworte extrem präzise und knackig. Nutze HTML-Tags (<b>, <br>).
        Struktur:
        1. STATUS (Potenzial?)
        2. RISIKEN (Gefahren?)
        3. IDEEN (Produktvorschläge?)`;

        // Wir nutzen das stabile Modell gemini-1.5-flash
        ...
        // Prüfung auf API-Fehler (z.B. Key ungültig oder Server überlastet)
        if (data.error) {
            let errorMsg = data.error.message;
            if (data.error.code === 503) {
                errorMsg = "Google Server überlastet (High Demand). Bitte in 1-2 Minuten erneut versuchen.";
            }
        ...
        // Formatierung für den HUD-Look
        // Ersetzt Markdown-Sterne durch Jarvis-Bolding
        ...
        resultsDiv.innerHTML = `
            <div class="border-l-2 border-l-green-500 bg-black/60 p-4 leading-relaxed font-mono shadow-[0_0_20px_rgba(0,255,65,0.1)] relative">
                ...
                <div class="mt-6 pt-2 border-t border-green-900/30 text-[9px] text-green-700 italic">
                    Analysis complete. Neural net uplink finalized.
                </div>
            </div>
        `;

        // Input nach Erfolg leeren
        inputField.value = "";
        ...
        resultsDiv.innerHTML = `
            <div class='p-4 border border-red-900 bg-red-900/10 text-red-500 font-mono text-[10px]'>
                ...
                <div class='mt-4 p-2 bg-black/40 border border-red-500/30 text-white'>
                    RECOVERY_PROCEDURE:<br>
                    1. Falls Code 503: Einfach kurz warten und erneut probieren.<br>
                    2. Prüfe den Key im SYSTEM-Reiter.<br>
                    3. Stelle sicher, dass Gemini 1.5 Flash in deinem Google AI Studio aktiv ist.
                </div>
            </div>`;
    }
}
